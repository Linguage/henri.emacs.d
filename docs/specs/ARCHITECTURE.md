...
6. `init-org.el` 中 roam 加载先于 academic，确保 `org-roam-directory` 不被 academic 覆盖；Academic 只在 Org-roam 已加载时配置 `org-roam-extra-files`。
7. Org academic 的 `org-academic-init` 通过 `after-init-hook` 延迟执行，避免与 roam 初始化竞争。
