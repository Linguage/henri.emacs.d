...
(defun henri/large-file-optimizations ()
  ...
  (when (and (boundp 'henri-large-file-threshold)
    ...
    (when (and (memq 'eglot henri-large-file-disable-modes)
      ...
      (message "[henri] Shut down eglot for large file: %s" (buffer-name)))
...
(defun henri/restore-from-large-file ()
  ...
  (let ((managed (and (buffer-local-boundp 'eglot--managed-mode (current-buffer))
                      (buffer-local-value 'eglot--managed-mode (current-buffer)))))
    (when (and (fboundp 'eglot-ensure) (not managed))
      (eglot-ensure)))
  (when (boundp 'treesit-font-lock-feature-list)
