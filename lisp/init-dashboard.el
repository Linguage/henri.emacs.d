  ...
  (let ((buffer (get-buffer-create "*Henri Dashboard*"))
    ...
    (with-current-buffer buffer
      (let ((inhibit-read-only t))
        ...
        (insert "\n  Notes Sync (Git)\n")
        (henri/dashboard-insert-action "        ⇣ Pull all notes repos" #'henri/notes-pull-all)
        (henri/dashboard-insert-action "        ⇡ Sync all (commit + push)" #'henri/notes-sync-all)
        (henri/dashboard-insert-action "        ✦ Status overview" #'henri/notes-status-all)
        (henri/dashboard-insert-action "        ✚ Magit status (pick repo)" #'henri/notes-magit)
        (insert "\n  Shortcuts\n")
        ...
        (insert "  C-c g N u  Notes: pull all\n")
        (insert "  C-c g N s  Notes: sync all\n")
        (insert "  C-c g N m  Notes: pick repo → magit-status\n")
